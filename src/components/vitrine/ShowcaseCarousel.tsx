...
function cardTransform(offset: number, dragPx = 0): string {
  ...
  if (offset === 0) {
    ...
    transform = `translate(-50%, -50%) translateX(${direction * 42}%) translateZ(-100px) rotateY(${direction * -30}deg) scale(0.86)`;
  } else if (Math.abs(offset) === 2) {
    const direction = offset > 0 ? 1 : -1;
    transform = `translate(-50%, -50%) translateX(${direction * 68}%) translateZ(-200px) rotateY(${direction * -42}deg) scale(0.74)`;
  } else {
    const direction = offset > 0 ? 1 : -1;
    transform = `translate(-50%, -50%) translateX(${direction * 82}%) translateZ(-280px) rotateY(${direction * -50}deg) scale(0.64)`;
  }

  if (dragPx === 0) return transform;
  return transform.replace("translate(-50%, -50%)", `translate(-50%, -50%) translateX(${dragPx}px)`);
}

function cardOpacity(offset: number, dragProgress: number): number {
  const distance = Math.abs(offset - dragProgress);
  if (distance < 0.35) return 1;
  if (distance < 1.35) return 0.5;
  if (distance < 2.35) return 0.26;
  return 0.1;
}

function cardZIndex(offset: number, dragProgress: number): number {
  return 40 - Math.round(Math.abs(offset - dragProgress) * 10);
}
...
function ProjectCard({
  ...
  dragPx: number;
  isDragging: boolean;
  blockNavigate: React.MutableRefObject<boolean>;
}) {
  const accent = showcaseAccent(item.url);
  const isActive = offset === 0;
  const label = item.name || showcaseHostname(item.url);
  const stageWidth = typeof window !== "undefined" ? window.innerWidth : 390;
  const dragProgress = stageWidth > 0 ? dragPx / (stageWidth * 0.45) : 0;
  ...
  return (
    <a
      ...
      draggable={false}
      onClick={(event) => {
        if (blockNavigate.current) {
          event.preventDefault();
          blockNavigate.current = false;
        }
      }}
...
export function ShowcaseCarousel({ items }: { items: ShowcaseCardData[] }) {
  ...
  const onPointerUp = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (axisLock.current === "x" && pointerStart.current) {
      const deltaX = event.clientX - pointerStart.current.x;
      const width = stageRef.current?.clientWidth ?? window.innerWidth;
      const threshold = Math.max(36, width * 0.14);
              ...
              >
                {items.map((item, index) => (
                  <ProjectCard
                    ...
                    blockNavigate={blockNavigateRef}
                  />
                ))}
              </div>
