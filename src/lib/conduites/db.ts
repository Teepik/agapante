...
async function backfillPassengers() {
  ...
  await sql.query(`
    ...
    FROM conduites_trips t JOIN conduites_memberships m ON m.group_id = t.group_id JOIN conduites_children c ON c.membership_id = m.id
...
export async function autoRegister(groupId: string, opts: { tripId?: string; childId?: string } = {}) {
  await q(`
    INSERT INTO conduites_passengers (id, trip_id, child_id)
    SELECT gen_random_uuid()::text, t.id, c.id
    FROM conduites_trips t
    JOIN conduites_memberships m ON m.group_id = t.group_id
    ...
    WHERE t.group_id = $1 AND t.date >= CURRENT_DATE AND NOT t.cancelled
      ...
      AND ($2::text IS NULL OR t.id = $2) AND ($3::text IS NULL OR c.id = $3)
    ON CONFLICT DO NOTHING`, [groupId, opts.tripId ?? null, opts.childId ?? null]);
}
